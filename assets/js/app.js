(() => {
  ...
  function jikanUrl(kind,slug,opt,page){const isAnime=kind==='anime',ids=JIKAN_GENRES[kind],rule=JIKAN_RULES[slug]||{genre:'isekai'},p=new URLSearchParams({sfw:'true',page:String(page),limit:'25'}),gid=ids[rule.genre];if(gid)p.set('genres',String(gid));const sort=Array.isArray(opt.sort)?opt.sort[0]:opt.sort;if(sort==='START_DATE_DESC'){p.set('order_by','start_date');p.set('sort','desc')}else if(sort==='POPULARITY_DESC'||sort==='TRENDING_DESC'){p.set('order_by','members');p.set('sort','desc')}else{p.set('order_by','score');p.set('sort','desc')}if(!isAnime){if(opt.format==='NOVEL')p.set('type','lightnovel');else if(opt.country==='KR')p.set('type','manhwa');else if(opt.country==='CHINESE')p.set('type','manhua');else if(opt.country==='JP')p.set('type','manga')}return `https://api.jikan.moe/v4/${isAnime?'anime':'manga'}?${p}`}
  async function fetchJikanOne(kind,slug,opt={}){const rule=JIKAN_RULES[slug]||{genre:'isekai'};let rows=[];for(let page=1;page<=2;page++){const j=await jikanGet(jikanUrl(kind,slug,opt,page));rows.push(...(j.data||[]).map(x=>normalizeJikan(x,kind==='anime')).filter(m=>jikanMatches(m,rule)));if(rows.length>=25||!j.pagination?.has_next_page)break}return rows}
  async function fetchJikanCategory(slug,opt={}){const type=opt.type||'ALL';let rows=[];if(type==='ALL'||type==='MANGA')rows.push(...await fetchJikanOne('manga',slug,opt));if((type==='ALL'||type==='ANIME')&&(!opt.country||opt.country==='JP'))rows.push(...await fetchJikanOne('anime',slug,opt));const map=new Map();rows.forEach(m=>map.set(`${m.type}:${m.id}`,m));return [...map.values()]}
  async function fetchResilientCategory(slug,opt={}){try{return {rows:await fetchCategoryDisplay(slug,opt),source:'anilist'}}catch(primary){console.warn('AniList failed, using Jikan',primary);return {rows:await fetchJikanCategory(slug,opt),source:'jikan'}}}
